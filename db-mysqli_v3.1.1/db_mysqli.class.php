 ...
 * @license BSD License
 * @copyright 2009 - // date('Y') \\ Camilo Sperberg -- $Date: 2011-07-07 08:56:03 -0400 (Thu, 07 Jul 2011) $
 */
...
class db_mysqli {
   ...
   * @param boolean $in_transaction Defaults to FALSE
   */
  ...
  public function __call($func,$arg_array) {
    ...
    switch ($func) {
      ...
      case 'version':
        ...
        else $result = $this->db->client_info;
        return $result;
      default:
        return 'Method not supported!';
        break;
    }
    if ($log) $this->logMe(self::$stats,$arg_array,$result,$this->error,$this->load_from_cache);
    return $result;
  }
  ...
  private function execute_result_info($arg_array = NULL) {
    if (!$this->error) {
      if ($this->cache_query === FALSE AND $this->load_from_cache === FALSE) {
        if ($this->db->affected_rows > 0) $num_rows = $this->db->affected_rows;
        else {
          if (isset($this->db->num_rows)) $num_rows = $this->db->num_rows;
          else $num_rows = 0;
        }
...
/**************************************************************************/
  ...
  private function liveStats ($query, $stats = NULL, $data = 0, $error = FALSE, $from_cache = FALSE) {
    ...
    $this->dbLiveStats[] = array('query' => $query,'number_results' => $results,'time'  => $stats['time'].' (seg)','memory' => $stats['memory'].' (bytes)','datasize' => $data.' (bytes)','error' => $error,'from_cache' => $valid_cache,'within_transaction' => $in_trans);
    return TRUE;
  }
